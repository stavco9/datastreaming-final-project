...
with open(os.path.join(DATA_DIR, "alternateNamesV2.txt")) as fr:
    ...
    for line in lines:
        ...
        isolanguage = line[2]
        alternatename = line[3] if len(line) > 3 else None
        if isolanguage == "link" and alternatename:
    ...
    for line in lines:
        ...
        isolanguage = line[2]
        alternatename = line[3] if len(line) > 3 else None
        if geonameid in dGeoWikititle and not isolanguage in geoPseudocodes and alternatename:
...
try:
    ...
    if wikiDB.is_connected() and gazDB.is_connected():
        ...
        for result in results:
        	if not any(substring in result['page_title'].decode() for substring in filter_out):
	            gtName = result['gt_name'].decode() if result['gt_name'] is not None else None
	            mainPageId = result['page_id']
	            mainPageTitle = result['page_title'].decode()
	            mainPageLen = result['page_len']
	            gtId = result['gt_id']
	            gtLat = result['gt_lat']
	            gtLon = result['gt_lon']
	            gtDim = result['gt_dim']
	            gtType = result['gt_type'].decode() if result['gt_type'] is not None else None
	            gtCountry = result['gt_country'].decode() if result['gt_country'] is not None else None
	            gtRegion = result['gt_region'].decode() if result['gt_region'] is not None else None
